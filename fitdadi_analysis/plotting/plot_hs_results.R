...
datafiles <- list.files(pattern="DFE_inference_discrete_")
...
dominance_plot <- function(data, main_text, axis, labels){
  plot(c(1,2,3,4,5),data[1,7:11],col=8,ylab="Dominance coefficient (h)",xlab="Selection coefficient bin",type="l", main=main_text, xaxt="n",cex.axis=axis, cex.lab=labels, cex.main=1.5)
...
proportion_plot <- function(data, axis, labels){
  ...
  #plot the values of h that can work:
  plot(c(1,2,3,4,5),data[1,2:6],col=8,ylim=c(0,0.4),ylab="Proportion of new mutations",xlab="Selection coefficient bin",type="l", xaxt="n", cex.axis=axis, cex.lab=labels, cex.main=1.5)
  for(i in c(2:length(data[,1])))
  {
...
pdf("~/Documents/UCLA/dominance/analysis/plots/plot_hs_models.pdf", width = 12, height = 10)
...
abline(v = -1453.0139, col="red", lwd=3, lty=2)
...
hist(data_highLL$Likelihood, xlab="Log-likelihood",main="", xlim=c(min(data$Likelihood), max(data$Likelihood)+5), cex.axis=axis, cex.lab=lab)
abline(v = -1453.0139, col="red", lwd=3, lty=2)
...
hist(data_highLL_monotonic$Likelihood, xlab="Log-likelihood",main="", xlim=c(min(data$Likelihood), max(data$Likelihood)+5), cex.axis=axis, cex.lab=lab)
abline(v = -1453.0139, col="red", lwd=3, lty=2)
...
#strongly recessive
strRec <- data[data$h5==0.5 & data$h4==0.25 & data$h3==0.15 & data$h2==0.05 & data$h1==0.00,]
strRec

# moderately recessive
modRec <- data[data$h5==0.5 & data$h4==0.35 & data$h3==0.25 & data$h2==0.15 & data$h1==0.05,]
modRec

# weakly recessive
wkRec <- data[data$h5==0.5 & data$h4==0.45 & data$h3==0.35 & data$h2==0.25 & data$h1==0.10,]
wkRec

# additive or recessive
addRec <- data[data$h5==0.5 & data$h4==0.50 & data$h3==0.50 & data$h2==0.50 & data$h1==0.00,]
addRec
